...
impl<K> UnionFind<K, EmptyMetadata>
where
  K: Clone + Eq + Hash,
{
  ...
  {
    Self::from_initializers(keys.into_iter().map(|key| (key, EmptyMetadata())))
  }
}
  ...
  M: Add<Output = M> + Clone,
{
  ...
  pub fn union(&mut self, k1: K, k2: K) -> K {
    let p1 = self.get_root(k1);
    let p2 = self.get_root(k2);
